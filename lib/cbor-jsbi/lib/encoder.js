...
class Encoder extends stream.Transform {
  ...
  _pushJSBigint(obj) {
    ...
    if (this.collapseBigIntegers && obj <= BI.MAXINT64) {
      ...
      return (
        this._pushUInt8((m << 5) | NUMBYTES.EIGHT) &&
...
Call removeLoopDetectors before resuming.`);
  ...
  static _pushDate(gen, obj) {
    switch (gen.dateType) {
      case "string":
        return (
          gen._pushTag(TAG.DATE_STRING) && gen._pushString(obj.toISOString())
        );
      case "int":
        return (
          gen._pushTag(TAG.DATE_EPOCH) &&
          gen._pushIntNum(Math.round(obj.getTime() / 1000))
        );
      case "float":
        // Force float
        return (
          gen._pushTag(TAG.DATE_EPOCH) && gen._pushFloat(obj.getTime() / 1000)
        );
      case "number":
      default:
        ...
        return (
          gen._pushTag(TAG.DATE_EPOCH) && gen.pushAny(obj.getTime() / 1000)
        );
    }
  }
  ...
  static encodeIndefinite(gen, obj, options = {}) {
    ...
    if (objType === "string") {
      ...
      ret =
        ret &&
        Encoder.pushArray(gen, obj, {
          indefinite: true,
        });
    } else if (obj instanceof Map) {
      ret =
        ret &&
        Encoder._pushMap(gen, obj, {
          indefinite: true,
        });
    } else {
      if (objType !== "object") {
